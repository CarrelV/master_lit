...
def zeroshot_classifier(model, tokenizer, classnames, templates):
    ...
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            texts = tokenizer(texts, padding=True,return_tensors="pt").to(device) #tokenize
...
def get_text_weights(model,tokenizer,class_set,no_prompt_eng = False):
    ...
    zeroshot_weights = zeroshot_classifier(model, tokenizer,classes, prompt_templates)
    zeroshot_weights = zeroshot_weights.permute(1, 0).float()
    print(zeroshot_weights.shape)

    return zeroshot_weights,len(classes)

def get_image_weights(model):

    image_weights_1 = model.image_projection.projection.weight
    image_weights_2 = model.image_projection.fc.weight

    return {image_weights_1,image_weights_2}
