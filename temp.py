...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, adapter_config=None):
        ...
        self.train_side_transformer = adapter_config.train_side_transformer if adapter_config is not None else False

        if self.train_side_transformer:
            side_config = copy.deepcopy(config)

            side_config.encoder_ffn_dim = side_config.encoder_ffn_dim // self.adapter_config.task_reduction_factor
            ...
            if side_layers is None:
                # add every layer
                side_layers = list(range(config.encoder_layers))

            self.side_block = nn.ModuleList(
                [BartEncoderLayer(side_config)
    ...
    def forward(
        ...
        if self.train_side_transformer:
            side_hidden_states = self.side_first_downsample(F.dropout(hidden_states, p=self.dropout, training=self.training))
        else:
            side_hidden_states = None
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.train_side_transformer:
                side_layer_module = self.side_block[idx]
                side_downsample = self.side_downsamples[idx]
            else:
                side_layer_module = None
                side_downsample = None
                    ...
                    if side_layer_module is not None:
                        if not self.add_residual_after:
                            if self.use_gate == "learnable":
                                ...
                                gate = getattr(self, f"side_gate_{idx}")
                        ...
                        if output_attentions:
                            all_side_attentions = all_side_attentions + (side_layer_outputs[1],)
                    else:
                        if output_attentions:
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, encoder_states, all_attentions, side_hidden_states, side_encoder_states, all_side_attentions] if v is not None)
        return SideBaseModelOutput(
            last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions, last_side_hidden_state=side_hidden_states, side_hidden_states=side_encoder_states, side_attentions=all_side_attentions
        )
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig, adapter_config=None):
        super().__init__(config)

        padding_idx, vocab_size = config.pad_token_id, config.vocab_size
        self.shared = nn.Embedding(vocab_size, config.d_model, padding_idx)

        self.encoder = BartEncoder(config, self.shared, adapter_config=adapter_config)
    ...
    def forward(
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
            ...
            encoder_outputs = SideBaseModelOutput(
                ...
                last_side_hidden_state=encoder_outputs[3] if len(encoder_outputs) > 3 else None,
                side_hidden_states=encoder_outputs[4] if len(encoder_outputs) > 4 else None,
                side_attentions=encoder_outputs[5] if len(encoder_outputs) > 5 else None,
            )
...
class BartForSequenceClassification(BartPretrainedModel):
    def __init__(self, config: BartConfig, **kwargs):
        ...
        self.model = BartModel(config, adapter_config=adapter_config)
    ...
    def forward(
        ...
        if self.train_side_transformer:
            hidden_states = outputs.last_side_hidden_state  # last side hidden state

            hidden_states = self.side_final_upsample(hidden_states)
        else:
        ...
        return Seq2SeqSequenceClassifierOutput(
            ...
            encoder_last_hidden_state=outputs.last_side_hidden_state,
            encoder_hidden_states=outputs.side_hidden_states,
            encoder_attentions=outputs.side_attentions,
        )
